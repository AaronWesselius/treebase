...
-->
	...
	<p>Please enter a name for your submission to TreeBASE: something that briefly describes what your submission is about. For example, "Phylogeny of Primates Based on Five Genes." Optionally, you can also write some notes for yourself, e.g. "still waiting on fifth dataset"</p>
	...
	<% } %>
  	  ...
  	  <c:if test="${publicationState eq 'NotReady'||isEditable eq 'yes'}">
    		<c:choose>
    			...
    			<c:otherwise>
    				<input class="styled-button" type="submit" name="Update" value="<fmt:message key="button.update"/>"/>
    				<!--input type="submit" name="Delete" value="<fmt:message key="button.delete"/>"/-->
    			</c:otherwise>
    		</c:choose>
    		<input class="styled-button-150" type="reset" name="Reset" value="<fmt:message key="button.reset"/>"/>
